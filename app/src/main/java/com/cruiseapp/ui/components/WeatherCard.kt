package com.cruiseapp.ui.components
...
import com.cruiseapp.data.remote.ForecastResponse
import com.cruiseapp.util.wmoToDescription
import com.cruiseapp.util.wmoToEmoji

@Composable
fun WeatherCard(forecast: ForecastResponse?, loading: Boolean, error: String?, onRetry: ()->Unit, modifier: Modifier=Modifier){
    Card(modifier.fillMaxWidth(), elevation=CardDefaults.cardElevation(4.dp)){
        Column(Modifier.padding(16.dp)){
            Text("Weather Forecast", style=MaterialTheme.typography.titleMedium)
            Spacer(Modifier.height(8.dp))
            when{
                loading -> { CircularProgressIndicator(); Spacer(Modifier.height(8.dp)); Text("Fetching from Open-Meteo (no API key)...", style=MaterialTheme.typography.bodySmall) }
                ...
                forecast == null -> Text("No data. Tap refresh when online.")
                else -> {
                    ...
                    if(cur!=null){
                        Row(Modifier.fillMaxWidth(), horizontalArrangement=Arrangement.SpaceBetween){
                            Column{
                                ...
                                Text("${cur.temperature2m ?: "--"}°C  • Wind ${cur.windSpeed?: "--"} km/h")
                            ...
                            Card(Modifier.fillMaxWidth().padding(vertical=4.dp), colors=CardDefaults.cardColors(containerColor=MaterialTheme.colorScheme.surfaceVariant)){
                                Row(Modifier.padding(12.dp).fillMaxWidth(), horizontalArrangement=Arrangement.SpaceBetween){
                                    ...
                                    Column(horizontalAlignment=androidx.compose.ui.Alignment.End){
                                        ...
                                        wind?.let{ Text("Wind ${it.toInt()} km/h", style=MaterialTheme.typography.bodySmall)}
                                    }
                                }
                            }
                        }
                    ...
                    Spacer(Modifier.height(8.dp))
                    Text("Source: Open-Meteo.com • CC BY 4.0 • Free, no API key", style=MaterialTheme.typography.bodySmall, color=MaterialTheme.colorScheme.onSurfaceVariant)
                }
            }
        }
    }
