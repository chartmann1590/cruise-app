...
class WeatherViewModel(app: Application): AndroidViewModel(app){
        ...
        viewModelScope.launch {
            ...
            if(result.isSuccess){
                ...
                if(data!=null){
                    ...
                    db.weatherCacheDao().insert(com.cruiseapp.data.local.WeatherCache(portStopId=portId, fetchedAt=System.currentTimeMillis(), json=jsonStr, tempMax=data.daily?.tempMax?.firstOrNull(), tempMin=data.daily?.tempMin?.firstOrNull(), weatherCode=data.current?.weatherCode, summary=data.daily?.weatherCode?.firstOrNull()?.toString() ?: ""))
                }
            } else {
    ...
    fun searchPlaces(query: String, onResult: (List<com.cruiseapp.data.remote.GeocodingResult>)->Unit, onError:(String)->Unit){
        viewModelScope.launch {
