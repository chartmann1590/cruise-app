...
fun WeatherDetailScreen(
    ...
    Scaffold(topBar={
        TopAppBar(title={ TText(port?.name ?: "Weather")}, navigationIcon={ IconButton(onClick=onBack){ Icon(Icons.Default.ArrowBack,null)}}, actions={ IconButton(onClick={ port?.let{ weatherVm.loadForPort(it.id, it.latitude, it.longitude); weatherVm.loadPlacesForPort(it.id, it.latitude, it.longitude) }}){ Icon(Icons.Default.Refresh,null)}})
